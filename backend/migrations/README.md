...
| 文件 | 用途 |
| --- | --- |
| `release.sql` | 可独立导入的上线 SQL，按顺序合并基础表、001 和 002，支持空库初始化与本项目旧版本升级 |
| `verify.sql` | 只读核对 8 张表及所需字段，返回数据库、schema、缺失项和状态 |
...
推荐在部署新服务前执行 `release.sql`。已有项目库无需重新建库或清空数据；重复执行会跳过已存在的表和字段。不要再额外执行一遍 001 / 002。
...
`verify.sql` 应返回 8 行 `OK`，`missing_columns` 均为 `{}`。它只检查表和字段是否存在，不会修改数据，也不保证已有字段类型、唯一约束、外键等定义一致。`IF NOT EXISTS` 不会修复手工改过的同名对象；这类库应与 `release.sql` 中的定义单独比对。
