...
export default function Dashboard({ user, token, accounts, setAccounts, route }) {
  ...
  useEffect(()=>{setAccountPageIndex(1)},[accountQuery,groupFilter,pageSize]);
  useEffect(() => {
  ...
  function updateAccount(id, value) {
    ...
    setError(''); setMessage('user_id' in value ? '账号所属用户已更新' : 'renewal_enabled' in value ? '是否续订已更新' : 'billing_address_id' in value ? '账号账单地址绑定已更新' : '账号付款卡绑定已更新');
  }
  ...
  const displayedPage = admin && accountSort.key === 'proxy' ? accountProxyPage(accounts, { config: proxyConfig, environmentID: id => browserEnvironmentID(user.id, id), query: accountQuery, group: groupFilter, direction: accountSort.direction, page: accountPageIndex, pageSize }) : accountPage;
