...
import ConsumptionOrders from './ConsumptionOrders';
...
import { browserEnvironmentID, launcherPort, launcherRequest } from './localBrowser';
...
import { renewalDays } from './accountRenewalStatus.mjs';
...
const accountPlanLabels = { plus: 'PLUS', pro_5x: 'PRO-5X', pro_20x: 'PRO-20X' };

function formatAccountPlan(plan) {
  return accountPlanLabels[String(plan || '').toLowerCase()] || '未设置';
}
...
export default function Dashboard({ user, token, accounts, setAccounts, route }) {
  ...
  const displayedPage = admin && accountSort.key === 'proxy' ? accountProxyPage(accounts, { config: proxyConfig, environmentID: id => browserEnvironmentID(user.id, id), query: accountQuery, group: groupFilter, renewalStatus, today, direction: accountSort.direction, page: accountPageIndex, pageSize }) : accountPage;
  const visibleAccounts = displayedPage.accounts;
  ...
  return <main className="dashboard">
    ...
    {tab === 'consumption-orders' ? <ConsumptionOrders token={token} /> : tab==='payment-exceptions' ? <PaymentExceptions token={token}/> : tab === 'orders' || tab==='packages' ? <RechargeManager key={tab} token={token} accounts={accounts} packagesOnly={tab==='packages'}/> : ['notices','audit','proxy-activity'].includes(tab) ? <OperationHistory key={tab} token={token} mode={tab}/> : tab === 'users' ? user.role === 'super_admin' ? <UserManager token={token} /> : <section className="account-section"><h2>无权访问用户列表</h2><p className="muted">只有超级管理员可以管理用户角色。</p><Link to="/admin/accounts" className="outline small">返回账号管理</Link></section> : tab === 'addresses' ? <AddressManager token={token} /> : tab === 'bank-cards' ? <BankCardManager token={token} accounts={accounts} /> : tab === 'proxies' ? <ProxyManager config={proxyConfig} error={proxyError} refresh={refreshProxies} onChange={setProxyConfig} /> : tab === 'wallet' ? <WalletPanel wallet={wallet} token={token} refresh={refresh} user={user || {}} notice={notice} /> : <section className="account-section">
      ...
      <div className="address-search"><input aria-label="搜索账号" placeholder="搜索账号邮箱或名称" value={accountQuery} onChange={e=>setAccountQuery(e.target.value)}/>{admin && <><button className="outline small" onClick={()=>downloadCSV('/accounts/export?'+new URLSearchParams({q:accountQuery,group:groupFilter==='all'?'':groupFilter===''?'none':groupFilter,renewal_status:renewalStatus,sort:accountSort.key === 'proxy' ? 'id' : accountSort.key,direction:accountSort.direction}),token,'账号清单.csv').catch(e=>setError(e.message))}>导出清单</button></>}</div><div className="account-toolbar">{admin && <><Select label="筛选账号分组" value={groupFilter} onChange={setGroupFilter} options={[{ value: 'all', label: '全部分组 · ' + accounts.length }, { value: '', label: '未分组 · ' + accounts.filter(account => !account.group_id).length }, ...groups.map(group => ({ value: String(group.id), label: group.name + ' · ' + group.account_count }))]} /><button className="outline small" onClick={() => setGroupManager(true)}>管理分组</button></>}<div className="account-renewal-filters" role="group" aria-label="按续费时间筛选">{[{ value: 'safe', label: '正常（>10 天）' }, { value: 'soon', label: '即将续费（0–10 天）' }, { value: 'overdue', label: '已逾期' }].map(status => <button key={status.value} type="button" className="outline small" data-status={status.value} aria-pressed={renewalStatus === status.value} onClick={() => { setRenewalStatus(current => current === status.value ? '' : status.value); setAccountPageIndex(1); }}>{status.label}</button>)}</div><span className="muted">{displayedPage.total} 个账号</span></div>
      <DataTable searchQuery={accountQuery} label="ChatGPT 账号列表" className={admin ? 'accounts-table' : 'accounts-table user-accounts-table'} sort={accountSort} onSort={sortAccounts} stickyActions={admin} columns={[{ label: '账号', key: 'account' }, '累计消费 USD', ...(admin ? [{ label: '所属用户', key: 'owner' }, '产品选型', { label: '分组', key: 'group' }, { label: 'SOCKS5', key: 'proxy', disabled: !proxyConfig }, { label: '续订日期', key: 'renewal_date' }, { label: '是否续订', key: 'renewal_enabled' }, { label: '付款卡', key: 'payment_card' }, '账单地址'] : [{ label: '续费日期', key: 'renewal_date' }, '套餐类型']), { label: '上次登录（UTC+8）', key: 'last_login_at' }, ...(admin ? ['操作'] : [])]} empty={!visibleAccounts.length && (accounts.length ? (accountQuery || renewalStatus ? '没有匹配的账号。' : '此分组暂无账号。') : <div className="empty"><KeyRound size={24} /><h3>还没有添加账号</h3><p>粘贴 Session JSON，保存你的 ChatGPT 账号。</p><button className="outline" onClick={() => open('add')}>添加第一个账号</button></div>)}>
        {visibleAccounts.map(account => <tr className="account-row" key={account.id}>
          ...
          <td><strong>{formatCardUSD(account.spent_usd_minor || 0)}</strong></td>
          ...
          {!admin && <><td><strong>{account.renewal_date || '未设置'}</strong></td><td><strong>{formatAccountPlan(account.subscription_plan)}</strong></td></>}
