...
export default function TwoFactor({ token, onVerify, onCancel, compact = false, verifyLabel = '验证并打开浏览器' }) {
  ...
  async function submit(event) {
    ...
    try {
      if (!enabled && !setup) {
        ...
        setMessage('验证器已绑定。继续操作时请使用下一组新验证码。');
      } else {
        await onVerify(code);
      }
    } catch (e) { setError(e.message); } finally { setBusy(false); setCode(''); }
  }
  return <form className={'two-factor' + (compact && enabled ? ' two-factor-compact' : '')} onSubmit={submit}>
    {enabled === null ? <p className="muted">正在读取验证器状态…</p> : !enabled && !setup ? <><p>请先绑定验证器，敏感操作需要两步验证。</p><label>当前登录密码<input type="password" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} required disabled={busy} /></label></> : !enabled ? <><p>使用 Google Authenticator、Microsoft Authenticator 等扫描二维码，10 分钟内输入验证码完成绑定。</p><img className="totp-qr" src={qr} alt="验证器绑定二维码" /><details><summary>无法扫码？手动输入密钥</summary><code className="totp-secret">{setup.secret}</code></details></> : !compact && <p className="success">两步验证已启用</p>}
