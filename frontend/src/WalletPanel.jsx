  ...
  return <section className="wallet-panel" aria-label="我的钱包">
    <div className="wallet-grid">
      <div className="wallet-balance"><span className="eyebrow">AITOK WALLET</span><h2>你的每一份灵感，都有余量。</h2>
        ...
        <p>1 美元 = {wallet.tokens_per_usd} 代币</p>
        <div className="wallet-rule">账号续订：{wallet.renewal_token_cost} 代币 / {wallet.renewal_months} 个月</div>
      </div>
    ...
    {history === 'orders' && <div className="table-wrap"><table><thead><tr><th>时间 / 订单号</th><th>单价</th><th>数量</th><th>总金额</th><th>代币</th><th>状态</th><th>操作</th></tr></thead><tbody>{wallet.orders.map(order => <tr key={order.order_no}><td>{new Date(order.created_at).toLocaleString('zh-CN')}<small className="order-number">{order.order_no}</small></td><td>{money(order.unit_amount_minor ?? order.amount_minor)}</td><td>{order.quantity ?? 1}</td><td>{money(order.amount_minor)}</td><td>{order.tokens}</td><td className={order.status === 'paid' ? 'credit' : ''}>{statuses[order.status] || order.status}</td><td>{order.status === 'pending' ? <button className="text-btn" disabled={!!syncing} onClick={() => syncOrder(order.order_no)}>{syncing === order.order_no ? '核对中…' : '核对支付'}</button> : '—'}</td></tr>)}</tbody></table>{wallet.orders.length === 0 && <p className="table-empty">暂无充值记录</p>}</div>}
    {history === 'renewals' && <div className="table-wrap"><table><thead><tr><th>扣款时间</th><th>ChatGPT 账号</th><th>续订时长</th><th>续订至</th><th>扣除代币</th><th>扣款后余额</th></tr></thead><tbody>{(wallet.renewals || []).map((item,index) => <tr key={index}><td>{new Date(item.created_at).toLocaleString('zh-CN')}</td><td>{item.account_label}<small className="order-number">账号 #{item.account_id}</small></td><td>{item.months ? item.months + ' 个月' : '—'}</td><td>{item.renewal_date}</td><td>-{item.tokens}</td><td>{item.balance_after ?? '—'}</td></tr>)}</tbody></table>{!wallet.renewals?.length && <p className="table-empty">暂无账号扣款记录，代币续订成功后会自动记录。</p>}</div>}
    {history === 'ledger' && <div className="table-wrap"><table><thead><tr><th>交易时间</th><th>类型</th><th>说明</th><th>代币变动</th><th>变动后余额</th></tr></thead><tbody>{wallet.ledger.map(item => <tr key={item.id}><td>{new Date(item.created_at).toLocaleString('zh-CN')}</td><td>{item.kind === 'stripe_topup' ? '充值入账' : item.kind === 'renewal' ? '账号续订' : item.kind}</td><td>{item.description}</td><td className={item.amount > 0 ? 'credit' : ''}>{item.amount > 0 ? '+' : ''}{item.amount}</td><td>{item.balance_after}</td></tr>)}</tbody></table>{wallet.ledger.length === 0 && <p className="table-empty">暂无钱包流水</p>}</div>}
  </section>;
}
