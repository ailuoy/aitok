...
export default function BankCardManager({ token, accounts = [] }) {
  ...
  return <section className="account-section bank-card-manager"><div className="section-title"><h2>银行卡管理 <span className="muted">{data.total} 张</span></h2><div className="browser-buttons"><button className="text-btn" disabled={loading} onClick={() => setRevision(value => value + 1)}><RefreshCw size={15} />刷新</button><button className="primary small" disabled={busy || archived || data.can_manage===false} onClick={() => edit()}><Plus size={15} />添加银行卡</button></div></div>
    ...
    <form className="address-search" onSubmit={event => { event.preventDefault(); setQuery(draft.trim()); setPage(1); }}><input aria-label="搜索银行卡" placeholder="搜索名称、持卡人、尾号、平台或备注" value={draft} maxLength={200} onChange={event => setDraft(event.target.value)} /><button className="outline small">搜索</button><Select label="银行卡归档筛选" value={String(archived)} onChange={v=>{setArchived(v==='true');setPage(1);}} options={[{value:'false',label:'当前卡片'},{value:'true',label:'已删除卡片（历史对账）'}]}/></form>
    {error && <p className="error" role="alert">{error}</p>}
    {loading ? <p className="muted">正在加载银行卡…</p> : <DataTable searchQuery={query} label="银行卡列表" className="bank-card-table" columns={['名称', '卡平台', '卡类型 / 尾号', '持卡人', '有效期', '可用 / 余额 · USD', '状态', '备注', '操作']} empty={!data.cards.length && '暂无银行卡，添加后可在账号浏览器中选择。'}>
      {data.cards.map(card => <tr className="bank-card-row" key={card.id}>
        <td className="table-text"><strong>{card.label}</strong></td><td className="table-text">{card.platform || '未设置'}</td><td className="table-mono">{card.brand} · •••• {card.last4}</td><td className="table-text">{card.cardholder}</td><td>{String(card.exp_month).padStart(2, '0')}/{String(card.exp_year).slice(-2)}</td><td>{formatCardUSD(card.balance_usd_minor-(card.reserved_usd_minor||0))}<small className="cell-secondary">余额 {formatCardUSD(card.balance_usd_minor)}</small></td><td>{card.deleted_at ? '已删除' : ({active:'正常',frozen:'冻结',invalid:'失效'}[card.status] || '正常')}</td><td className="table-text bank-card-notes">{card.notes || '—'}</td>
