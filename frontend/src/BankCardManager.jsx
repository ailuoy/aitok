...
export default function BankCardManager({ token, accounts = [] }) {
  ...
  useEffect(() => {
    ...
    request('/bank-cards?' + new URLSearchParams({ q: query, page, page_size: pageSize, archived:archived ? '1':'0' }), token, { signal: controller.signal })
      .then(data => { if (!controller.signal.aborted) setData(data); })
      .catch(error => { if (!controller.signal.aborted) setError(error.message); })
      .finally(() => { if (!controller.signal.aborted) setLoading(false); });
    return () => controller.abort();
  }, [token, query, page, pageSize, revision,archived]);
  function openEditor(value = {}, editToken) {
    ...
    setDialog({ type: 'edit', card: value, editToken });
  }
  function edit(card) {
    if (busy) return; setError(''); setFormError('');
    if (card) setDialog({ type: 'verify', card: { id: card.id } });
    else openEditor();
  }
  async function verifyEdit(totpCode) {
    setBusy(true);
    try {
      const data = await request('/bank-cards/' + dialog.card.id, token, { totpCode });
      openEditor(data.card, data.edit_token);
  ...
  async function submit(event) {
    ...
    try {
      ...
      let body;
      if (!deleting) {
        body = { ...Object.fromEntries(new FormData(event.currentTarget)), exp_month: Number(month), exp_year: Number(year), wallet_qr_image: walletQR };
        if (dialog.card.id) {
          body.edit_token = dialog.editToken;
          if (!body.number.trim() || body.number === dialog.card.number) delete body.number;
        }
        if (!body.cvc.trim()) delete body.cvc;
      }
      await request('/bank-cards' + (dialog.card.id ? '/' + dialog.card.id : ''), token, { method: deleting ? 'DELETE' : dialog.card.id ? 'PATCH' : 'POST', ...(deleting ? {} : { body }) });
      setDialog(null);
  ...
  return <section className="account-section bank-card-manager"><div className="section-title"><h2>银行卡管理 <span className="muted">{data.total} 张</span></h2><div className="browser-buttons"><button className="text-btn" disabled={loading} onClick={() => setRevision(value => value + 1)}><RefreshCw size={15} />刷新</button><button className="primary small" disabled={busy || archived || data.can_manage===false} onClick={() => edit()}><Plus size={15} />添加银行卡</button></div></div>
    ...
    {dialog?.type === 'verify' && <Dialog title="验证后编辑银行卡" onClose={() => { if (!busy) setDialog(null); }}><TwoFactor token={token} verifyLabel="验证并编辑银行卡" onVerify={verifyEdit} onCancel={() => setDialog(null)} /></Dialog>}
    {dialog && dialog.type !== 'verify' && <Dialog className="bank-card-dialog" title={dialog.type === 'delete' ? '删除银行卡' : dialog.card.id ? '编辑银行卡' : '添加银行卡'} onClose={() => { if (!busy && !imageBusy) setDialog(null); }}><form className={dialog.type === 'edit' ? 'bank-card-form' : undefined} onSubmit={submit}>
      {dialog.type === 'delete' ? <p>确认删除「{dialog.card.label} · 尾号 {dialog.card.last4}」？卡片将从列表隐藏，余额及对账记录会保留。</p> : <><label>名称<input name="label" defaultValue={dialog.card.label} required maxLength={80} autoFocus /></label><label>卡平台<Select name="platform" label="卡平台" value={platform} onChange={setPlatform} disabled={busy} options={[{ value: '', label: '未设置' }, ...Array.from(new Set([...(data.platforms || []), platform].filter(Boolean))).map(value => ({ value, label: value }))]} searchPlaceholder="输入或搜索卡平台…" createLabel="使用输入的平台" onCreate={value => { const name = value.trim().replace(/\s+/g, ' '); if (!name || Array.from(name).length > 80) { setFormError('请输入 1 至 80 字的卡平台名称'); return; } setPlatform(name); setFormError(''); }} /></label><label>持卡人姓名<input name="cardholder" defaultValue={dialog.card.cardholder} required maxLength={120} autoComplete="off" /></label><label>卡号<input name="number" type="text" inputMode="numeric" defaultValue={dialog.card.number || ''} required={!dialog.card.id} maxLength={24} autoComplete="off" onFocus={event => event.currentTarget.select()} />{dialog.card.id && <small className="muted">未修改或留空则保留原卡号；更换时请输入完整新卡号。</small>}</label><div className="proxy-fields"><label>到期月份<Select label="到期月份" value={month} onChange={setMonth} options={Array.from({ length: 12 }, (_, index) => ({ value: String(index + 1), label: String(index + 1).padStart(2, '0') }))} /></label><label>到期年份<Select label="到期年份" value={year} onChange={setYear} options={Array.from({ length: 26 }, (_, index) => ({ value: String(new Date().getFullYear() + index), label: String(new Date().getFullYear() + index) }))} /></label></div><label>CVC 安全码<input name="cvc" type="password" inputMode="numeric" defaultValue="" pattern="[0-9]{3,4}" maxLength={4} autoComplete="new-password" placeholder={dialog.card.id ? '不填写则不修改' : '3 或 4 位数字（选填）'} />{dialog.card.id && <small className="muted">不填写则保留原 CVC 安全码。</small>}</label><ImageUpload label="钱包地址二维码" value={walletQR} onChange={setWalletQR} onBusyChange={setImageBusy} disabled={busy} /><label>备注<textarea name="notes" defaultValue={dialog.card.notes} rows={3} maxLength={1000} placeholder="填写用途、使用限制等备注（选填）" /></label></>}
