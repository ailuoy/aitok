...
export default function BankCardManager({ token, accounts = [] }) {
  ...
  return <section className="account-section bank-card-manager"><div className="section-title"><h2>银行卡管理 <span className="muted">{data.total} 张</span></h2><div className="browser-buttons"><button className="text-btn" disabled={loading} onClick={() => setRevision(value => value + 1)}><RefreshCw size={15} />刷新</button><button className="primary small" disabled={busy} onClick={() => edit()}><Plus size={15} />添加银行卡</button></div></div>
    ...
    {loading ? <p className="muted">正在加载银行卡…</p> : <DataTable label="银行卡列表" className="bank-card-table" columns={['名称', '卡平台', '卡类型 / 尾号', '持卡人', '有效期', '余额 · USD', '备注', '操作']} empty={!data.cards.length && '暂无银行卡，添加后可在账号浏览器中选择。'}>
      {data.cards.map(card => <tr className="bank-card-row" key={card.id}>
        <td className="table-text"><strong>{card.label}</strong></td><td className="table-text">{card.platform || '未设置'}</td><td className="table-mono">{card.brand} · •••• {card.last4}</td><td className="table-text">{card.cardholder}</td><td>{String(card.exp_month).padStart(2, '0')}/{String(card.exp_year).slice(-2)}</td><td>{formatCardUSD(card.balance_usd_minor)}</td><td className="table-text bank-card-notes">{card.notes || '—'}</td>
        <td className="table-actions"><div className="row-actions"><button className="outline small" disabled={busy} onClick={() => setLedgerCard(card)}>余额 / 对账单</button><button className="outline small" disabled={busy} onClick={() => edit(card)}>编辑</button><button className="text-btn danger" disabled={busy} onClick={() => { setDialog({ type: 'delete', card }); setFormError(''); }}>删除</button></div></td>
      </tr>)}
    </DataTable>}
