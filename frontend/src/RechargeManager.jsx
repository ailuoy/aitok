import OrderWalletDebit from './OrderWalletDebit';
...
const actionLabels = { wallet_debit: '钱包扣款', record: '补录订单', refund: '登记客户退款', refund_note: '退款登记（余额未退回）', cancel: '取消订单', discard: '废弃订单' };
...
export default function RechargeManager({ token, packagesOnly = false, quickAccount = null, onQuickClose, onQuickCreated }) {
  ...
  const [walletOrder, setWalletOrder] = useState(null);
  ...
  return <>{!quickAccount && <section className="account-section">
    ...
    {walletOrder && <OrderWalletDebit order={walletOrder} token={token} onClose={() => setWalletOrder(null)} onComplete={() => { setWalletOrder(null); refresh(v => v + 1); }} />}
