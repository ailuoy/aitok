...
普通用户可查看和添加自己的 ChatGPT 账号，列表只展示账号与上次登录时间；也可通过右上角“我的 → 钱包与充值”查看本人代币余额、充值记录、历史账号扣款及全部流水，发起 Stripe 充值、核对本人支付并申请退款。钱包支持直接访问、刷新和支付回跳；服务端按登录用户隔离资金记录，普通用户不能访问他人的订单、退款审核或业务充值订单管理页。管理员可访问所有业务模块和账号数据；角色分配仍仅限环境配置的超级管理员。
