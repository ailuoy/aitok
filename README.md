...
1. 在访问网页的电脑上安装 AiTok 助手和 Chrome / Edge。桌面助手内置运行环境，不需要 Node.js、项目源码或终端；macOS 提供 Apple Silicon / Intel 安装包，Windows 提供 x64 安装程序。打包与安装说明见 [desktop/README.md](desktop/README.md)。
2. 打开桌面助手，默认添加线上站点 `https://toktopup.com`（本机端口 `15683`）和开发站点 `http://localhost:15680`（本机端口 `15684`）。支持添加、编辑、启停和移除站点。不同站点的端口不能重复，移除需二次确认并保留浏览器与代理数据。关闭助手窗口会隐藏到菜单栏 / 托盘；安装后可勾选“开机启动”。
...
桌面助手按站点维护独立监听器。线上默认 15683，localhost / 127.0.0.1 / ::1 开发站点默认 15684；多个开发站点可各自配置其他端口。网页的账号弹窗和 SOCKS5 管理均提供“本机助手连接”，端口设置保存在当前网页来源的 localStorage 中，不影响其他站点。助手和网页需要填写相同端口。此处是助手端口，不是前后端服务端口。
...
账号列表在付款卡后提供账单地址绑定，支持输入姓名、街道、城市或邮编搜索，也支持随机绑定；随机结果持久保存，删除地址时自动解除活跃账号绑定。小助手优先显示当前账号绑定的卡和地址，支持一键复制带字段名称及换行的卡号、有效期、安全码和完整账单地址；复制内容不进入操作审计。本机打开弹窗默认收起指纹和连接设置，展开后仍可查看全部指纹配置及重新生成。
