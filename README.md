...
从工作台重新“打开账号”后，ChatGPT 网页右侧显示可折叠的“账号助手 0.0.4”（版本号在脚本中统一定义），收起后的“账号助手”按钮可拖拽或使用方向键移动，窗口缩小与重新展开时自动限制在可视区域；刷新页面后恢复默认右侧位置。助手包含邮箱、真实登录状态、官网返回的套餐、银行卡和账单地址详情，以及复制和手动填充功能。已绑定项固定展示，只有未绑定项提供下拉选择；不再提供切换下一项或刷新资源按钮，需要重新读取时刷新官网页面。不包含删除登录状态、登录其他账号或获取充值队列。套餐按钮进入官网套餐选择页，并不直接创建订单或承诺某个方案可购买，实际方案与价格以官网为准。
