...
test('账号导入、后台管理与本机打开页面桌面、移动端冒烟测试', { skip: !process.env.AITOK_BROWSER_SMOKE, timeout: 90000 }, async t => {
  ...
  assert.equal(await evaluate('Boolean(document.querySelector(".workspace-tabs a[href$=wallet]"))'), false);
  ...
  await evaluate('document.querySelector(".workspace-tabs a[href$=accounts]").click()');
  ...
  await evaluate('document.querySelector(".workspace-tabs a[href$=addresses]").click()');
  await wait('document.querySelectorAll(".address-card").length === 20');
  assert.equal(await evaluate('Boolean(document.querySelector(".address-card a"))'), false);
  await evaluate('document.querySelector(".address-source-details summary").click()');
  ...
  await evaluate('document.querySelector(".address-source-details summary").click()');
  assert.deepEqual(await evaluate('Array.from(document.querySelectorAll(".address-card:first-child .address-details dt"), item => item.textContent)'), ['账单姓名', '街道地址', '城市', '州 / 省', '邮编', '国家 / 地区']);
  assert.equal(await evaluate('document.querySelector(".address-card").innerText.includes("未填写")'), true);
  await click('下一页');
  await wait('document.querySelectorAll(".address-card").length === 1');
  await click('添加地址');
  ...
  await click('保存地址');
  await wait('!document.querySelector("dialog") && document.querySelector(".address-card")?.innerText.includes("100 New Road")');
  await click('编辑');
  ...
  await click('保存地址');
  await wait('!document.querySelector("dialog") && document.querySelector(".address-card")?.innerText.includes("Unit 2")');
  assert.equal(await evaluate('document.querySelector(".address-card").innerText.includes("Updated User")'), true);
  ...
  await evaluate('document.querySelector(".address-card").scrollIntoView({block:"center"})');
  ...
  assert.equal(await evaluate('Boolean(document.querySelector(".workspace-tabs a[href$=addresses]"))'), true);
  assert.equal(exportCount, 0);
  await evaluate('document.querySelector(".workspace-tabs a[href$=bank-cards]").click()');
  ...
  await click('保存银行卡');
  await wait('!document.querySelector("dialog") && Boolean(document.querySelector(".bank-card"))');
  assert.equal(await evaluate('document.querySelector(".bank-card").innerText.includes("4242424242424242")'), false);
  assert.equal(bankCards[0].platform, '自定义卡平台');
  assert.equal(bankCards[0].notes, '月度订阅\n仅工作用途');
  assert.ok(await evaluate('document.querySelector(".bank-card").innerText.includes("自定义卡平台") && document.querySelector(".bank-card").innerText.includes("仅工作用途")'));
  await click('编辑');
  ...
  await click('保存银行卡');
  await wait('!document.querySelector("dialog") && document.querySelector(".bank-card").innerText.includes("工作卡已编辑")');
  ...
  await click('删除'); await click('确认删除');
  await wait('!document.querySelector("dialog") && !document.querySelector(".bank-card")');
  assert.deepEqual(bankWrites, ['POST', 'PATCH', 'DELETE']);
  await evaluate('document.querySelector(".workspace-tabs a[href$=proxies]").click()');
  ...
  await click('关闭');
  await click('删除'); await click('确认删除');
  await wait('!document.querySelector("dialog") && !document.querySelector(".proxy-card")');
  await click('添加代理');
  ...
  await click('保存代理');
  await wait('document.querySelector(".proxy-card")?.innerText.includes("美国代理")');
  await click('测试');
  ...
  await wait('!document.querySelector(".proxy-form .primary").disabled');
  await click('保存代理');
  await wait('document.querySelector(".proxy-card")?.innerText.includes("美国代理已编辑")');
  await evaluate('document.querySelector(".workspace-tabs a[href$=accounts]").click()');
  ...
  await evaluate('document.querySelector("dialog button[aria-label=关闭]").click()');
  await evaluate('document.querySelector(".workspace-tabs a[href$=proxies]").click()');
  await wait('Boolean(document.querySelector(".proxy-card"))');
  ...
  await wait('Boolean(document.querySelector(".proxy-card"))');
  await click('删除'); await click('确认删除');
  await wait('!document.querySelector(".proxy-card")');
  await evaluate('document.querySelector(".workspace-tabs a[href$=accounts]").click()');
