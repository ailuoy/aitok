...
function installPanel(version, isVerificationPage) {
  ...
  const mount = () => {
    ...
    function position() {
      ...
      panelDocument.body.classList.toggle('compact', compact);
      panel.style.maxWidth = '100%';
      panel.style.maxHeight = Math.max(1, viewportHeight - 24) + 'px';
      frame.style.width = (compact ? tab.getBoundingClientRect().width : Math.min(320, viewportWidth - 24)) + 'px';
      frame.style.height = Math.ceil((compact ? tab : panel).getBoundingClientRect().height) + 'px';
    ...
    tab.onpointerdown = event => {
      ...
      suppressClick = false;
      drag = { id: event.pointerId, x: event.screenX, y: event.screenY, right, top };
    ...
    let data = { cards: [], addresses: [] }, cardID = '', addressID = '', cardRequest = 0, selectedCard = null;
    const picker = (label, empty, onSelect) => {
      ...
      return { set(entriesValue, selected, bound) { details.hidden = Boolean(bound); if (bound) details.open = false; entries = bound ? [] : entriesValue; summary.textContent = entries.find(entry => String(entry.id) === String(selected))?.label || empty; render(); } };
    };
    const cards = picker('银行卡', '请选择银行卡', id => { cardID = id; void showCard(); });
    ...
    function addField(container, name, value) {
      const row = create('div', '', '', container); create('dt', name, '', row);
      ...
      button.disabled = !text; button.setAttribute('aria-label', '复制' + name);
      button.onclick = event => { if (event.isTrusted && text) void copyValue(name, text); };
    }
    ...
    async function showCard() {
      ...
      selectedCard = null; updateCopy();
      cardDetails.replaceChildren();
      if (!selected) { create('p', data.payment_card_id ? '账号绑定的付款卡不可用，请在后台检查绑定。' : '暂无银行卡，请先在后台添加。', 'hint', cardDetails); return; }
      ...
      try {
        ...
        const sample = testCode(card);
        addField(cardDetails, !card.cvc && sample ? '测试安全码' : '安全码', card.cvc || sample);
        updateCopy();
    ...
    function showAddress() {
      ...
      updateCopy();
      if (!address) { addressDetails.replaceChildren(); if (data.billing_address_id) create('p', '账号绑定的地址不可用，请在后台检查绑定。', 'hint', addressDetails); return; }
    ...
    function updateCopy() { copyPayment.disabled = !selectedCard || !data.addresses.some(item => item.id === addressID); fill.disabled = copyPayment.disabled; }
    copyPayment.onclick = event => {
      ...
      const fields = [
        ...
        ['安全码', selectedCard.cvc || testCode(selectedCard)],
      ];
    ...
    function updatePickers() { cards.set(data.cards.map(card => ({ id: card.id, label: card.label + ' · ' + card.brand + ' •••• ' + card.last4 })), cardID, data.payment_card_id); addresses.set(data.addresses.map(address => ({ id: address.id, label: address.address_line1 + ', ' + address.city + ', ' + address.state + ' ' + address.postal_code })), addressID, data.billing_address_id); }
    ...
    position();
    load().then(result => { message.textContent = result.message; }).catch(error => { message.textContent = error.message; });
