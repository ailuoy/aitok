...
function installPanel(version) {
  ...
  const mount = () => {
    ...
    const label = create('label', '安全码（本次使用，不保存）', '', panel); const cvc = create('input', '', '', label); cvc.type = 'text'; cvc.inputMode = 'numeric'; cvc.maxLength = 4; cvc.autocomplete = 'off'; cvc.placeholder = '输入所选银行卡的安全码';
    ...
    function updateCode() {
      ...
      codeField?.(cvc.value || sample, sample && !cvc.value ? '测试安全码' : '安全码');
    }
    async function showCard() {
      const request = ++cardRequest, selected = cardID;
      selectedCard = null; codeField = null;
    ...
    fill.onclick = event => { if (!event.isTrusted) return; const value = cvc.value || testCode(selectedCard); cvc.value = ''; updateCode(); run(fill, () => call('fill', { card_id: cardID, address_id: addressID, cvc: value })); };
